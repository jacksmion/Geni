            ...
            <div className="flex items-center gap-2 p-1.5 pl-4 bg-white/70 dark:bg-[#1c1c1f]/80 backdrop-blur-2xl border border-slate-200/60 dark:border-white/10 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.12)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.3)]">
                ...
                <div className="flex items-center gap-1">
                    <button
                        onClick={onReset}
                        disabled={isSaving}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[11px] font-semibold text-slate-500 hover:text-slate-700 dark:text-zinc-400 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-white/5 transition-all disabled:opacity-50"
                    >
                        <RotateCcw size={12} />
                        {t('settings.reset')}
                    </button>
                    <button
                        onClick={onSave}
                        disabled={isSaving}
                        className="flex items-center gap-1.5 px-5 py-1.5 rounded-xl bg-indigo-600 text-white text-[11px] font-bold hover:bg-indigo-700 shadow-md shadow-indigo-600/20 active:scale-95 transition-all disabled:opacity-50 disabled:shadow-none min-w-[70px] justify-center"
                    >
                        {isSaving ? (
